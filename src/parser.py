...
def _parse_linha_tropa(linha):
    ...
    try:
        ...
        if len(info_tropa) != 2:
            print(
                f"AVISO: Formato de info de tropa inválido: '{parte_info}'. Esperado '<id> <forca>'. Linha ignorada."
            )
            return None
        ...
        print(
            f"AVISO: Erro de formatação na linha de tropa: '{linha}'. Linha ignorada."
        )
        return None
...
def _parse_linha_transporte(linha):
    ...
    try:
        ...
        padrao = re.compile(
            r"missao:\s*COLETAR\s+(MAX|\d+)\s+DE\s+([\w_]+),\s*ENTREGAR\s+EM\s+([\w_]+)",
            re.IGNORECASE,
        )
        ...
        if not match:
            print(
                f"AVISO: Formato de missão de transporte inválido: '{linha}'. Linha ignorada."
            )
            return None
        ...
        print(
            f"AVISO: Erro de formatação na linha de transporte: '{linha}'. Linha ignorada."
        )
        return None
