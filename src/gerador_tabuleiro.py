...
class Grafo:
    ...
    def adicionar_aresta(self, no_a: str, no_b: str, peso: int):
        """
        ...
        for jogador in range(num_jogadores):
            self.nos[f"basej_{jogador}"] = {
                "pos": rotate(
                    (largura // 2, altura // 2),
                    (RAIO, altura // 2),
                    ((math.pi * 2) / num_jogadores) * jogador,
                ),
                "pop": 100,
                "owner": jogador,
            }
        ...
        while True:
            ...
            for camada_idx in range(1, len(regioes_jogadores[0])):
                ...
                for cidade_idx, nome_cidade in enumerate(camada_atual):
                    ...
                    num_conexoes = random.randint(1, min(4, len(conexoes)))
                    conexoes_escolhidas = [
                        camada_anterior[random.randint(0, len(camada_anterior) - 1)]
                    ]
                    conexoes.remove(
                        conexoes_escolhidas[0]
                    )  # Remove a conexão já escolhida
                    conexoes_escolhidas.extend(
                        random.sample(conexoes, num_conexoes - 1)
                    )
                    ...
                    for i in range(len(conexoes_escolhidas)):
                        ...
                        if i == num_conexoes - 1:
                            ...
                            peso = random.randint(
                                peso_medio // 90, peso_medio + (peso_medio // 10)
                            )
                        pesos.append(peso)
            ...
            for i in range(num_jogadores):
                for j in range(i + 1, num_jogadores):
                    ...
                    num_ligacoes = min(
                        2, len(camada_i), len(camada_j)
                    )  # exemplo: 2 conexões por par

                    # Escolhe pares de cidades para conectar
                    pares = random.sample(
                        [(c1, c2) for c1 in camada_i for c2 in camada_j], num_ligacoes
                    )
...
def exportar_mapa_para_json(grafo, nome_arquivo):
    ...
    if os.path.exists(nome_arquivo):
        resposta = (
            input(f"O arquivo '{nome_arquivo}' já existe. Deseja sobrescrever? (s/n): ")
            .strip()
            .lower()
        )
        if resposta != "s":
